...
public class MainDashboard implements Initializable {
    ...
    public TextField meetingSearchTxt;
    public Button meetingSearchButton;
    ...
    public void addButtonClick(ActionEvent actionEvent) throws IOException {
        ...
        Stage stage = (Stage) ((Button) actionEvent.getSource()).getScene().getWindow();
        stage.setTitle("Add New Meeting");
        stage.setScene(new Scene(root));
        stage.show();
    }
    ...
    public void modifyButtonClick(ActionEvent actionEvent) throws SQLException {
        try {
            ...
            Stage stage = (Stage) ((Button) actionEvent.getSource()).getScene().getWindow();
            stage.setTitle("Modify Existing Meeting");
            stage.setScene(new Scene(root));
            stage.show();
        } catch (NullPointerException e) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText("Error");
            alert.setTitle("No meeting selected.");
            alert.setContentText("Please selected meeting to modify.");
            alert.show();
        } catch (IOException e) {
    ...
    public void meetingSearchButtonClick(ActionEvent actionEvent) {
        try {
            String search = meetingSearchTxt.getText();
            ObservableList<Appointment> appointment = Appointment.searchForAppointment(search);
            if (appointment.size() == 0) {
                int apptID = Integer.parseInt(search);
                Appointment searchedAppointment = Appointment.searchForAppointment(apptID);
                if (searchedAppointment != null) {
                    appointment.add(searchedAppointment);
                }
            }
            apptTable.setItems(appointment);
        } catch (NumberFormatException e) {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setHeaderText("Error");
            alert.setContentText("No matching meetings found.");
            alert.show();
        }
    }
    ...
    public void deleteButtonClick(ActionEvent actionEvent) throws SQLException {
        ...
        if (deleteAPPT == null) {
            ...
            alert.setContentText("Please select meeting for deletion.");
            alert.show();
        } else {
            // PER EVALUATOR REPORT: added appt ID and appt type to delete message
            Alert confirm = new Alert(Alert.AlertType.CONFIRMATION);
            confirm.setTitle("Confirm Delete Action");
            confirm.setHeaderText("Are you sure you want to delete this meeting?");
            confirm.setContentText("Meeting ID: " + deleteAPPT.getApptID() + "   |   Meeting Type: " + deleteAPPT.getType());
            Optional<ButtonType> result = confirm.showAndWait();
            if (result.isPresent() && result.get() == ButtonType.OK) {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // CHANGED THIS HERE
        descColumn.setCellValueFactory(new PropertyValueFactory<>("customerName"));
